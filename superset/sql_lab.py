...
def handle_error(msg, query, session, payload):
...
def execute_sql_statement(
    ...
    if (parsed_query.is_select() and SQL_MAX_ROWS and
...
def execute_sql_statements(
    ...
    with closing(engine.raw_connection()) as conn:
        with closing(conn.cursor()) as cursor:
            ...
            for i, statement in enumerate(statements):
                ...
                logging.info(f'Starting statement {i+1} out of {statement_count}')
    ...
    # Update the query entry in database
    query.rows = cdf.size
    query.progress = 100
    ...
    if store_results:
        ...
        with stats_timing('sqllab.query.results_backend_write'):
